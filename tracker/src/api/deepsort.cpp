    ...
    for(int iloop=0;iloop<detect_objects.size();iloop++)
    {
        ...
        temp_object.tlwh = DETECTBOX(
            ...
            try{
                ...
                if(false){ // change it to use with hog
                    float *feature;
                    feature = get_hog_feature(img(cv::Rect(detect_objects[iloop].rect.x,detect_objects[iloop].rect.y,detect_objects[iloop].rect.width,detect_objects[iloop].rect.height)));
                ...
                {
                    // std::cout << "------------feature taking using the model started\n\n\n\n\n" << std::endl;
                //    feature =  FeatureTensor::getInstance()->getRectsFeature_single(img, temp_object);
                   temp_object.feature = FEATURE(FeatureTensor::getInstance()->getRectsFeature_single(img, temp_object));
                }
